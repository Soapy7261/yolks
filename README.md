...
If a folder named 'mongodb-data-to-restore' exists in the /home/container directory mongoDB will try to restore all the data from that folder, this is useful for restoring a backup or migrating data, but this will drop existing data in the database, so be careful!
